...
from src.Backend import colorize as colorize, Image_editor as ImEd

# Setting our Color Pallet
...
img = ImEd.create_image(
    size=(1440, 1440), color=color, sfile=Path("data/images/Test-Image1.png")
)

# Loading a preferred font with a specified size. Could load a default font by keeping file_name = None
font = ImEd.load_font(
    file_name=Path("data/Fonts/DancingScript.ttf"), size=img.width * 9 / 100
)
...
img = ImEd.add_text(
    img=img,
    text=q["content"],
    text_box_width=img.width * 70 / 100,
    ...
    h_tbalign="left",
    v_tbalign="center",
    h_indent=img.width * 7 / 100,
    v_indent=-(img.height * (7 / 100)),
    nline_text=[f"        - {q["author"]}"],
)
